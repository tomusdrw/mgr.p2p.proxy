...
if __name__ == '__main__':
    ...
    try:
        nodes.append(os.spawnlp(os.P_NOWAIT, 'python2', 'python2', '../entangled/node.py', str(startPort)))
        for i in range(amount-1):
            ...
            output = '\r[%-50s] %d/%d' % (hashbar, i, amount)
            sys.stdout.write(output)
            nodes.append(os.spawnlp(os.P_NOWAIT, 'python2', 'python2', '../entangled/node.py', str(port), ipAddress, str(startPort)))
            port += 1
